...
function targets(){if(slimeMob()){tx=innerWidth*.5;tyBase=innerHeight-92;ty=tyBase+follow();return}var s=spots[live<0?0:live%spots.length];tx=innerWidth*s[0];tyBase=innerHeight*s[1];ty=tyBase+follow()}
var forceT=null;
...
slime.style.transform='translate3d('+(sx-33.5)+'px,'+(sy-33.5+bob)+'px,0)';
...
bubble.style.top=(sy-56-bubble.offsetHeight+bob)+'px';
bubble.style.borderRadius='18px 18px 18px 18px';
menu.style.left=Math.max(14,Math.min(innerWidth-238,sx-112))+'px';
menu.style.top=(sy-58-menu.offsetHeight)+'px';
}else{
...
bubbleDY=best}
var saidOnce={};
function say(txt){if(!txt||saidOnce[txt])return;saidOnce[txt]=1;bubble.textContent=txt;bubble.classList.add('is-on');bubbleT=performance.now()+5200;bubbleDY=0;clearTimeout(fitT);fitT=setTimeout(fit,780)}
