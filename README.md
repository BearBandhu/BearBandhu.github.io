...
2. Upload every file in this folder to the repository root — including the two dotfiles, `.nojekyll` and `.image-slots.state.json`. (GitHub's drag-and-drop upload works; so does `git push`.)
3. Repo → **Settings → Pages** → Source: *Deploy from a branch*, Branch: `main`, Folder: `/ (root)` → **Save**.
4. Wait ~1 minute. The URL appears at the top of that Settings → Pages screen.

`.nojekyll` matters: without it GitHub runs Jekyll, which strips dotfiles and would drop the image state file.
...
- `art/`, `assets/lylyland/` — images
- `*.css`, `*.js` — styles and behaviour (scroll deck, slime companion, contact button)
- `.image-slots.state.json` — the screenshots dropped into image slots on the project pages
